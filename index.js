...
program
    ...
    .addOption(
        new Option("-i, --index", "Create an index.js file for the component").default(true),
    );
...
if (changed)
    logs.logComponentNameChanged(utils.getNameFromArray(COMPONENT_NAME_ARRY), COMPONENT_NAME);
...
if (!inProject && !FORCE) {
    ...
    const fixs = [
        chalk.gray("Fix: ") +
            chalk.gray("Please make sure you are running this in the root of your react project"),
...
function createComponent() {
    ...
    const component_dir = path.join(constants.COMPONENTS_PATH, COMPONENT_NAME);
    ...
    try {
        ...
        if (ADD_INDEX) {
            const indexPath = path.join(component_dir, "index.js");
            const indexTemplate =
                "export * from './COMPONENT_NAME';\nexport { default } from './COMPONENT_NAME';";
            const index = indexTemplate.replaceAll("COMPONENT_NAME", COMPONENT_NAME);
